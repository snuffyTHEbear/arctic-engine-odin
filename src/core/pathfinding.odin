...
find_path :: proc(pf: ^Pathfinder, iso_map: ^world.IsoMap, start, end: Point) -> [dynamic]Point {
	slice.fill(pf.g_score, 999999.0)
	slice.fill(pf.f_score, 999999.0)
	slice.fill(pf.in_open_set, false)
	slice.fill(pf.came_from, -1)

	start_idx := world.get_tile_index(iso_map, start.x, start.y)
	end_idx := world.get_tile_index(iso_map, end.x, end.y)
	...
	for len(open_set) > 0 {
		...
		if current_idx == end_idx {
			return reconstruct_path(pf, iso_map, current_idx)
		}

		unordered_remove(&open_set, current_list_idx)
		pf.in_open_set[current_idx] = false

		cx := current_idx % iso_map.width
		cy := current_idx / iso_map.width

		neighbours := [4]Point{{cx + 1, cy}, {cx - 1, cy}, {cx, cy + 1}, {cx, cy - 1}}

		for n in neighbours {
			if n.x < 0 || n.x >= iso_map.width || n.y < 0 || n.y >= iso_map.height {
				continue
			}

			n_idx := world.get_tile_index(iso_map, n.x, n.y)
			height_diff := math.abs(
				iso_map.tile_heights[n_idx] - iso_map.tile_heights[current_idx],
			)
...
reconstruct_path :: proc(
	...
	for curr != -1 {
		x := curr % iso_map.width
		y := curr / iso_map.width
