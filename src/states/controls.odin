...
update_controls :: proc(controls: ^Controls, dt: f32) {
	...
	if rl.IsKeyPressed(.C) {
		controls.palette_idx = (controls.palette_idx + 1) % len(utils.THEMES)
	}
	...
	if !controls.paused {
		if rl.IsKeyDown(.KP_3) do controls.speed += 1.0 * dt
		if rl.IsKeyDown(.KP_1) do controls.speed -= 1.0 * dt
